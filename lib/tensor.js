...
class TensorData {
    ...
    constructor(M) {
        ...
        this._rotmat.set(evecs[0][0], evecs[1][0], evecs[2][0], 0,
            evecs[0][1], evecs[1][1], evecs[2][1], 0,
            evecs[0][2], evecs[1][2], evecs[2][2], 0,
            0, 0, 0, 1);
        this._quat = new THREE.Quaternion();
        this._quat.setFromRotationMatrix(this._rotmat);

        var hevecs = mjs.transpose(this._haeb_evecs);
        this._haeb_rotmat = new THREE.Matrix4();
        this._haeb_rotmat.set(hevecs[0][0], hevecs[1][0], hevecs[2][0], 0,
            hevecs[0][1], hevecs[1][1], hevecs[2][1], 0,
            hevecs[0][2], hevecs[1][2], hevecs[2][2], 0,
            0, 0, 0, 1);
        this._haeb_quat = new THREE.Quaternion();
        this._haeb_quat.setFromRotationMatrix(this._haeb_rotmat);
    }
