...
AtomImage.prototype = {
    ...
    set visible(v) {
        ...
        if (v) {
            ...
            for (let n in this._labels) {
                let l = this._labels[n];
    ...
    addLabel(text, name, parameters = {}) {
        ...
        parameters.position = this.xyz; // This is not customizable

        var label = new TextSprite(text, parameters);
        this._labels[name] = label;

        this._model._renderer._addSprite(label);
    },
    /**
    ...
    removeLabel(name) {
        ...
        if (l)
            this._model._renderer._removeSprite(l);
