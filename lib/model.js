 ...
 */
function Model(atoms, supercell = [3, 3, 3]) {
    ...
    */
    ...
    this._atom_images = this.atomImages(this._supercell); 
...
Model.prototype = {
    ...
    _computeMolecules() {
        ...
        var links = JSON.parse(JSON.stringify(this._bonds)); // Fast deep copy hack
        for (var i = 0; i < this.length; ++i) {
    ...
    setBondsVisibility(aimages, value) {
        ...
        for (var i = 0; i < aimages.length; ++i) {
            ...
            for (var j = i + 1; j < aimages.length; ++j) {
                ...
                var ijk2 = im2.ijk;
                var ijk1 = im1.ijk;
                var c = [ijk2[0] - ijk1[0], ijk2[1] - ijk1[1], ijk2[2] - ijk1[2]];
...
function AtomImage(model, index, ijk) {
    ...
    this._radius = this.vdwRadius / 4.0;
    this._opacity = 1.0;
}
...
AtomImage.prototype = {
    ...
    set visible(v) {
        ...
        } else if (v && this._renderer) {
            this._mesh = this._renderer._addAtom(this._xyz,
                this._radius,
                this._color);
        }
    }
