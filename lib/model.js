...
AtomImage.prototype = {
     ...
     * @param {Array}   position Position of the label (relative to the atom)
     ...
     */
    addLabel(text, position, name, size=1, options={}) {
        if (name[0] == '#') {
        ...
        if (this._model._renderer) {
            position = mjs.add(position, this.xyz);
            var label = this._model._renderer._addBillBoard(position, text, size);
