...
if (typeof BUILD_GRAPHICS !== 'undefined' && BUILD_GRAPHICS) {
    // Conditional to allow testing of this module without building
...
class AtomImage {
    ...
    get mesh() {
        if (!this._mesh) {
            this._mesh = new Primitives.AtomMesh(this._xyz, this.radius, this._color);
    ...
    get aura() {
        if (!this._aura) {
            this._aura = new Primitives.AuraMesh({
                radius: this.radius,
                scale: 0.02
            });
    ...
    addLabel(text, name, parameters = {}) {
        ...
        var label = new Primitives.TextSprite(text, parameters);
    ...
    addEllipsoid(data, name, parameters = {}) {
        ...
        var ellips = new Primitives.EllipsoidMesh(parameters);
        this._ellipsoids[name] = ellips;
        this._mesh.add(ellips);
    }
...
class BondImage {
    ...
    get mesh() {
        if (!this._mesh) {
            this._mesh = new Primitives.BondMesh(this.atom1.xyz, this.atom2.xyz,
                this._radius,
...
class Model {
     ...
     *                                          - supercell
     *                                          - molecular_crystal (if true, load full molecules in central unit cell)
     */
    constructor(atoms, parameters = {}) {
        ...
        const init_molecules = ((atoms, supercell) => {
        ...
        init_molecules(atoms, parameters.supercell);

        if (parameters.molecular_crystal) {
            ...
            for (let i = 0; i < this.length; ++i) {
                ...
                for (let j = 0; j < mol.length; ++j) {
                    var a = mol[j];
                    if (a.index == i) {
                        pos[i] = mjs.add(pos[i], this.frac2abs(a.cell));
                    }
                }
            }
        ...
        if (r) {
            ...
            if (this.periodic) {
                ...
                if (!this.cartesian_box) {
                    this.cartesian_box = new Primitives.BoxMesh(this.cell);
                }
                if (!this.cartesian_axes) {
                    this.cartesian_axes = new Primitives.AxesMesh(this.cell, {
                        linewidth: 1.5
                    });
                }
