...
Renderer.prototype = {
    ...
    /**
     * Add an atom or a bond to the rendered model
     * 
     * @param {AtomMesh | BondMesh} obj          Atom or bond to add
     *
     */
    _addAtomBond: function(obj) {
        // Avoid duplication
        if (!this._g._ab.children.includes(obj))
            this._g._ab.add(obj);
    },
    /**
     * Add lattice box
     * 
     * @param {BoxMesh | AxesMesh} l   Lattice axes or box
     */
    _addLattice: function(l) {

        if (!this._g._latt.children.includes(l))
            this._g._latt.add(l);
    },
    /**
     * Add 2D sprite
     * 
     * @param {ImageSprite} sprite       Sprite to add
     */
    _addSprite: function(sprite) {

        if (!this._g._sprites.children.includes(sprite))
            this._g._sprites.add(sprite);
    },
    /**
     * Add a sprite that always faces the camera and always has the same size
     * 
     * @param {LabelMesh} bb     LabelMesh object to add
     */
    _addBillBoard: function(bb) {

        if (!this._g._bboards.children.includes(bb))
