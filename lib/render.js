...
function Renderer(target, width, height) {
    ...
    this._sboxlist = [];
    this._sbox = new SelectionBox(this._c, this._g._ab);
    this._sboxhelp = new SelectionHelper(this._sbox, this, 'crystvis-selbox-helper');
    this._sboxhelp.selectOverCallback = this._selectBox.bind(this);
    ...
    this.selbox_bkg_color = 0xaa0000;
    this.selbox_border_color = 0xff0000;
...
Renderer.prototype = {
    ...
    _raycastClick: function(e) {
        ...
        for (var i = 0; i < this._rcastlist.length; ++i) {
            ...
            var rcast = this._rcastlist[i];
            var targ = rcast[1] || this._s;
            var intersects = raycaster.intersectObjects(targ.children);

            var objects = [];
            for (var j = 0; j < intersects.length; ++j) {
                var o = intersects[j].object;
                if (!rcast[2] || intersects[j].object instanceof rcast[2]) {
                    objects.push(o);
                }
            }
    ...
    _selectBox: function(selected) {
        for (var i = 0; i < this._sboxlist.length; ++i) {
            this._sboxlist[i][0](selected);
        }
    },
    /**
    ...
    /**
     * Add a listener for selection box events (atoms only)
     * 
     * @param {Function} listener Listener function
     */
    addSelBoxListener: function(listener) {
        var sbl = [listener];
