...
function _parseOneAtomLine(line, units, labels) {
    ...
    var sp = line[0];
    var sp_i = parseInt(line[1]);
    var i = _.findIndex(labels, function(x) {
        return (x[0] == sp) && (x[1] == sp_i);
    });
    var ltrim = Array.from(line).splice(2);
...
function _parseTwoAtomLine(line, units, labels) {
    var sp1 = line[0]
    var sp1_i = parseInt(line[1]);
    var i1 = _.findIndex(labels, function(x) {
        return (x[0] == sp1) && (x[1] == sp1_i);
    });
    var ltrim = Array.from(line).splice(2);

    var ans = _parseOneAtomLine(ltrim, units, labels);
...
Loader.prototype = {
    ...
    loadXYZ(contents) {
        ...
        var a = new Atoms(elems, pos, cell, info);
        if (ext) {
    ...
    loadMagres(contents) {
        ...
        var v_re = /#\$magres-abinitio-v([0-9]+\.[0-9]+)/;
        var v_match = lines[0].match(v_re);
        ...
        for (var i = 1; i < lines.length; ++i) {
            var b_match = lines[i].match(b_re);
            if (b_match) {
                // Start or end?
                if (b_match[1] == '/') {
        ...
        // Process each block
        for (var bname in blocks) {
            var block = blocks[bname];
        ...
        if ('lattice' in ablock) {
            var u = 1.0;
            var uname = ablock.lattice.units;
            if (uname) {
            ...
            cell = [];
            var line = Array.from(ablock.lattice.lines[0]);
            for (var i = 0; i < 3; ++i) {
        ...
        if ('atom' in ablock) {
            ...
            for (var i = 0; i < ablock.atom.lines.length; ++i) {
                var l = ablock.atom.lines[i];
                elems.push(l[0]);
        ...
        // Create atoms object
        var atoms = new Atoms(elems, pos, cell, {
        ...
        var mblock = blocks.magres;

        if (mblock) {

            for (var tag in mblock) {
                var tag_type = tag.split('_')[0];

                var u = 1.0;
                var uname = mblock[tag].units;
                if (uname) {
                ...
                switch (_MagresParsers[tag_type]) {
                    ...
                    case _parseTwoAtomLine:
                        tag_dimension = 2;
                        tag_data = _.times(N, function(x) {
                            return new Array(N);
                        });
                }

                for (var i = 0; i < mblock[tag].lines.length; ++i) {
                    var l = mblock[tag].lines[i];
                    var data = _MagresParsers[tag_type](l, u, labels);
